(ns funeral.operation
  "OperationActor -- one funeral/death-care operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (FuneralOps-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Funeral Services Governor
  ...
  One graph run = one funeral/death-care operation (intake -> advise ->
  ...
  decision to a human operator (the licensed funeral director). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:disposition/perform` ALWAYS reaches this node when the
  governor is clean -- see `funeral.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [funeral.funeralopsllm :as funeralopsllm]
            [funeral.governor :as governor]
...
(defn build
  ...
  `funeral.store/Store`).
  opts:
    :advisor      -- a `funeral.funeralopsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (funeralopsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; FuneralOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (funeralopsllm/-advise advisor store request)]
            {:proposal p :audit [(funeralopsllm/trace request p)]})))

      ;; Funeral Services Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
